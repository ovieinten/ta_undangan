...
@section('content')
    ...
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h4 class="text-themecolor">Diskon</h4>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="button-list">
                        <a onclick="addForm()" class="btn btn-success waves-effect w-md mr-2 mb-2 btn-rounded" style="float: right;" >Tambah Diskon</a>
                    </div>
                    <h4 class="card-title">Data Diskon</h4>
                    <h6 class="card-subtitle">Berisikan Data Diskon Produk</h6>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        var table = $('#table-layout').DataTable({
            processing: true,
            serverside: true,
            ajax: "{{ route('backend.discount.getdata') }}",
            columns: [
        ...
        function editForm(id) {
            ...
            $('#ps-form')[0].reset();
            $.ajax({
                url: "{{ url('b/discount') }}/" + id,
                type: "GET",
                dataType: "JSON",
                success: function (data) {
                    $('#modal-form').modal('show');
                    $('.modal-title').text('Edit Diskon');
                    $('#id').val(data.id);
        ...
        function viewForm(id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('.modal-title').text('Lihat Detail Diskon');
                    $('#d_product').text(data.product_id);
                    $('#d_percent').text(data.percent);
                    $('#d_date_start').text(data.date_start);
                    $('#d_date_end').text(data.date_end);
                },
                error: function (e) {
                    console.log(e);
                    alert("Nothing Data");
        ...
        $(function () {
            ...
            $('#ps-form').on('submit', function (e) {
                if (!e.isDefaultPrevented()) {
                        ...
                        if (save_method === "add") {
                            url = "{{url('b/discount/store')}}";
                            messages = "Ditambah";
                        }
                        else {
                            id = $('#id').val();
                            url = "{{ url('b/discount/update') }}/" + id;
                            messages = "Diubah";
                        }
                        table.ajax.reload();
                        Cooems.initSubmitAction(messages,url);
                        $("#parent_id").load("{{url('b/discount/select')}}");
                    return false;
                }
            });
        });
        ...
        function deleteRow(id){
            if (id) {
                var url =  "{{ url('b/discount/delete') }}/" + id;
