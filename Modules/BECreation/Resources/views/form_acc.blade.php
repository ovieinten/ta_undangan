...
@section('content')
    ...
    <form action="{{route('backend.creation.storeAcc', ['slug' => @$data['creation']->slug])}}" enctype="multipart/form-data" method="post" onsubmit="ShowLoading()">
        {{csrf_field()}}
            ...
            <div class="box-body" style="padding-bottom: 50px;">
                <div class="row">
                            ...
                            <div class="input-group">
                                <select required name="category" class="select-form-advance js-select2 form-control">
                                    ...
                                    @foreach(@$data['category'] as $item)
                                        <option value="{{@$item->id}}" {{old('category') == @$item->id ? "selected" : @$item->id == @$data['creation']->category_id ? 'selected' : null}}>{{@$item->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            <br>
                            <label>*Jangan lupa untuk memasukkan Kategori Karya yang akan menjadi Produk.</label>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <label>Note</label>
                            <textarea required id="note" name="note" style="width: 100%;">{{ @$data['creation']->note }}</textarea>
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                ...
                <!-- /.row -->
                <button class="btn btn-danger"><a href="{{ URL::previous() }}" style="color: white;">Kembali</a></button>
