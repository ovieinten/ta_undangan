...
@section('content')
    ...
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h4 class="text-themecolor">Ukuran</h4>
        </div>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="button-list">
                        <a onclick="addForm()" class="btn btn-success waves-effect w-md mr-2 mb-2 btn-rounded" style="float: right;" >Tambah Ukuran</a>
                    </div>
                    <h4 class="card-title">Data Ukuran</h4>
                    <h6 class="card-subtitle">Berisikan Data Ukuran Produk</h6>
...
@section('script')
    ...
    <script type="text/javascript">
        ...
        var table = $('#table-layout').DataTable({
            processing: true,
            serverside: true,
            ajax: "{{ route('backend.size.getdata') }}",
            columns: [
                {data: 'name', name: 'name'},
        ...
        function viewForm(id) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $('.modal-title').text('Lihat Detail Ukuran');
        ...
        $(function () {
            $('#ps-form').on('submit', function (e) {
                if (!e.isDefaultPrevented()) {
                    id = $('#id').val();
                        if (save_method === "add") {
                            url = "{{url('b/size/store')}}";
                            messages = "Ditambah";
                        }
                        else {
                            url = "{{ url('b/size/update') }}/" + id;
                            messages = "Diubah";
                        }
                        table.ajax.reload();
